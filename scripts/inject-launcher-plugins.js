...
// The shipped launcher is the Svelte build (src/launcher.html, generated by
// scripts/build-launcher.mjs); its plugin list lives in
// launcher-ui/src/legacy-saver.ts (DEFAULT_PLUGINS). This injector only
// maintains the frozen legacy launcher kept for parity reference.
const launcherPath = path.join(ROOT_DIR, 'assets', 'legacy-launcher.html');
...
function main() {
    ...
    if (matchCount === 0) {
        console.error("Error: Could not find 'const defaultPlugins = [...];' in assets/legacy-launcher.html to replace.");
        process.exit(1);
    }

    fs.writeFileSync(launcherPath, newLauncherContent);
    console.log(`Successfully injected ${plugins.length} plugins into ${matchCount} locations in assets/legacy-launcher.html.`);
}
