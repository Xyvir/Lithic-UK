...
| `src/launcher.html` | TRACKED **GENERATED** artifact — the shipped launcher, built from `launcher-ui/` by `scripts/build-launcher.mjs`. **Never hand-edit:** edit the Svelte sources and rebuild (`npm run build:launcher`). `npm run check:push` rebuilds and fails if the committed bytes have drifted. It is the ONLY launcher artifact: the Tauri bundle (`tauri.conf.json`), the smoke/Puppeteer tests, the preview workflow, the server image and `autoupdate.sh` all reference this one name. `src/lithic.html` sits beside it as the engine it fetches. |
