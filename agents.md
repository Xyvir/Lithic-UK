...
| `external.yml` + `external-lock.json` | TRACKED dependency manifest for third-party plugins (fetched fresh at build time). |
...
- The `TIDDLYWIKI_PLUGIN_PATH`/`TIDDLYWIKI_THEME_PATH` env vars in `.github/workflows/build-wiki.yml` point at the staging dirs `wiki/` + `wiki/plugins` — they exist only so the CLI can load the flattened plugins at compile time.
