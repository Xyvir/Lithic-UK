...
5. **Compilation:** The core `tiddlywiki` CLI is invoked to compile the output. For prod builds, the flibbles/uglify plugin is loaded and engaged via `npx tiddlywiki wiki --uglify --build index`, minifying core/plugin JS/CSS and the plugin JSON stores (~27% size reduction, 10.2 MB -> ~7.4 MB). Non-prod builds run plain `--build index` (generate-config.js strips flibbles/uglify from the pre/dev merged configs).
   ...
   - The CI 'Verify Uglified Artifact' step guards both properties: size must be below ~9.5 MB and the flibbles/uglify blob must be absent.
